...
/* Guardar bio (solo en memory localProfiles y no persistida por vendedor - la guardamos en localStorage dentro de reviews store para simplicidad) */
function saveBioForCurrent(){
  ...
  // Guardamos bio junto a reviews en localStorage (sencillo)
...
/* borrar reseñas de seller actual */
...
/* ===== añadir producto en localStorage ===== */
...
function setup(){
  ...
  form.addEventListener("submit", (e)=>{
    ...
    form.reset();
    document.getElementById("filter-category").value = ""; // opcional: reset filtro
  });
  ...
  // bio guardar y contacto
  document.getElementById("save-bio").addEventListener("click", saveBioForCurrent);
  document.getElementById("contact-seller").addEventListener("click", ()=> { /* definido en openProfile */ });
